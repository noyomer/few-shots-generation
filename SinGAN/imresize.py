...
def np2torch(imgs, opt):
    ...
    if opt.nc_im == 3:
        ...
        print("Please insert 3 channels images only")
...
def imresize(imgs_in,scale,opt):
    #s = im.shape
    # imgs_out = np.zeros((imgs_in.shape[0],
    #                      imgs_in.shape[2],
    #                      imgs_in.shape[3],
    #                      imgs_in.shape[1]))
    imgs_out = []
    n = imgs_in.shape[0] # num of images
    for i in range(n):
        im_out = imgs_in[i, :, :, :]
        im_out = torch2uint8(im_out)
        im_out = imresize_in(im_out, scale_factor=scale)
        imgs_out.append(im_out)
    imgs_out = np.array(imgs_out)
    imgs_out = np2torch(imgs_out, opt)
