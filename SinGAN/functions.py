...
def compare_image_sizes(heights, widths):
    """ Returns true if all images of the same size. Otherwise, returns false"""
    all_h = all(h == heights[0] for h in heights)
    all_w = all(w == widths[0] for w in widths)
    return (all_h and all_w)
...
def np2torch(imgs, opt):
    ...
    if opt.nc_im == 3:
        x = np.zeros((n, c, h, w))
        for i in range(n):
            img = imgs[i, :, :, :].transpose((2, 0, 1))/255
            x[i, :, :, :] = img[0:3, :, :]
    else:
        print("Please insert 3 channels images")
...
def read_image2np(opt):
    x = img.imread('%s/%s' % (opt.input_dir,opt.model_name))
    x = x[:, :, 0:3]
    return x
...
def generate_dir2save(opt):
    ...
    if (opt.mode == 'train') | (opt.mode == 'SR_train'):
        dir2save = 'TrainedModels/%s/scale_factor=%f,alpha=%d' % (opt.model_name, opt.scale_factor_init,opt.alpha)
    elif (opt.mode == 'animation_train') :
        dir2save = 'TrainedModels/%s/scale_factor=%f_noise_padding' % (opt.model_name, opt.scale_factor_init)
    elif (opt.mode == 'paint_train') :
        dir2save = 'TrainedModels/%s/scale_factor=%f_paint/start_scale=%d' % (opt.model_name, opt.scale_factor_init,opt.paint_start_scale)
    elif opt.mode == 'random_samples':
        dir2save = '%s/RandomSamples/%s/gen_start_scale=%d' % (opt.out,opt.model_name, opt.gen_start_scale)
    elif opt.mode == 'random_samples_arbitrary_sizes':
        dir2save = '%s/RandomSamples_ArbitrerySizes/%s/scale_v=%f_scale_h=%f' % (opt.out,opt.model_name, opt.scale_v, opt.scale_h)
    elif opt.mode == 'animation':
        dir2save = '%s/Animation/%s' % (opt.out, opt.model_name)
    elif opt.mode == 'SR':
        dir2save = '%s/SR/%s' % (opt.out, opt.sr_factor)
    elif opt.mode == 'harmonization':
        dir2save = '%s/Harmonization/%s/%s_out' % (opt.out, opt.model_name,opt.ref_name[:-4])
    elif opt.mode == 'editing':
        dir2save = '%s/Editing/%s/%s_out' % (opt.out, opt.model_name,opt.ref_name[:-4])
    elif opt.mode == 'paint2image':
        dir2save = '%s/Paint2image/%s/%s_out' % (opt.out, opt.model_name, opt.ref_name[:-4])
...
def post_config(opt):
    ...
    opt.out_ = 'TrainedModels/%s/scale_factor=%f/' % (opt.model_name, opt.scale_factor)
    if opt.mode == 'SR':
        opt.alpha = 100
