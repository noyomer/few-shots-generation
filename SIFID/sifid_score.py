...
def calculate_sifid_given_paths(path1, path2, batch_size, cuda, dims, suffix):
    ...
    for j in range(len(files1)):
        for i in range(len(files2)):
            m1, s1 = calculate_activation_statistics([files1[j]], model, batch_size, dims, cuda)
            m2, s2 = calculate_activation_statistics([files2[i]], model, batch_size, dims, cuda)
            curr_fid = calculate_frechet_distance(m1, s1, m2, s2)
            print("SIFID between {} and {} is: {:.7f}".format(files1[j].name[:-4],files2[i].name[:-4],curr_fid))
            fid_values.append(curr_fid)
