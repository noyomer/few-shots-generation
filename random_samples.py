...
if __name__ == '__main__':
    ...
    parser.add_argument('--zopt_index', type=int, help='Base noise map over few images', default=0)
    ...
    if dir2save is None:
        ...
        if opt.mode == 'random_samples':
            print('random samples for image %s, start scale=%d, already exist' % (opt.model_name, opt.gen_start_scale))
        elif opt.mode == 'random_samples_arbitrary_sizes':
            print('random samples for image %s at size: scale_h=%f, scale_v=%f, already exist' % (opt.model_name, opt.scale_h, opt.scale_v))
    else:
        try:
